...
import LinkMenu from "~/components/links_menu/LinkMenu";
...
const DashboardLinks = () => {
  const {setSessionId, userLinks, addLink} = useLinksStore(state => ({
  ...
  const handleDisplayNewLink = useCallback(() => {
      const id = generateUUID();
      addLink({id: id, platform: 'github', url: ""})
  ...
  const renderLinksContent = useMemo(() => {
        if (userLinks.length === 0) {
          return (
              <>
                <svg className={styles.empty_icons} xmlns="http://www.w3.org/2000/svg" width="250" height="161" fill="none" viewBox="0 0 250 161">
                  <path fill="#fff"
                ...
                <p>Use the “Add new link” button to get started. Once you have more than one link, you can reorder and edit
